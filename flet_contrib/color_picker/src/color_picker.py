...
class ColorPicker(ft.Column):
    ...
    def _before_build_command(self):
        ...
        self.update_color_field(self.hue_slider.hue)
    ...
    def update_color_field(self, hue):
        container_gradient_colors = [
            rgb2hex(colorsys.hsv_to_rgb(hue, 0, 1)),
            rgb2hex(colorsys.hsv_to_rgb(hue, 1, 1)),
        ]

        self.color_field.content.gradient.colors = container_gradient_colors
        new_color = rgb2hex(
            colorsys.hsv_to_rgb(hue, hex2hsv(self.color)[1], hex2hsv(self.color)[2])
        )
